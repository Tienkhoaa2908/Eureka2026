...
def validate_analysis_lock(lock_path: str | Path | None, core: str | Path, entry: str | Path) -> None:
    """Chỉ cho ước lượng khi cổng dữ liệu và đặc tả đã được ghi nhận, đúng hàm băm."""
    if lock_path is None:
        raise ValueError("Chưa khóa dữ liệu và đặc tả: cần --analysis-lock; không được chạy kết quả cuối.")
    lock = json.loads(Path(lock_path).read_text(encoding="utf-8"))
    if lock.get("status") != "LOCKED" or lock.get("data_gate_passed") is not True:
        raise ValueError("Bản khóa chưa đạt cổng dữ liệu/đặc tả")
    for key, path in (("core_sha256", core), ("entry_sha256", entry)):
        if lock.get(key) != hashlib.sha256(Path(path).read_bytes()).hexdigest():
            raise ValueError(f"Sai hàm băm bản khóa: {key}")
    hypotheses_hash = hashlib.sha256(json.dumps(PRIMARY_HYPOTHESES, sort_keys=True).encode()).hexdigest()
    if lock.get("hypotheses_sha256") != hypotheses_hash:
        raise ValueError("Giả thuyết đã thay đổi so với bản khóa")
    repo = Path(__file__).resolve().parents[2]
    for key, name in (("protocol_sha256", "docs/research/RESEARCH_PROTOCOL_SR1.md"),
                      ("dictionary_sha256", "data/metadata/DATA_DICTIONARY_SR1.csv")):
        if lock.get(key) != hashlib.sha256((repo / name).read_bytes()).hexdigest():
            raise ValueError(f"Tài liệu khác bản khóa: {name}")
...
def mechanism_decomposition(
    ...
    outcomes = [
        ...
        ("revenue_per_worker", "dlog_revenue_per_worker"),
    ]
    ...
    for component in ("cstp4", "cstp5"):
        ...
        # Khóa cùng tập tỉnh-năm cho mọi vế của đẳng thức, không lọc riêng mỗi vế.
        common_rows = [r for r in rows if r.get(exposure) is not None
                       and all(r.get(outcome) is not None for _, outcome in outcomes)]
        for margin, outcome in outcomes:
            fitted = fit_fe(common_rows, outcome, [exposure])
...
def run(core_csv: str | Path, entry_csv: str | Path, output_dir: str | Path,
        analysis_lock: str | Path | None = None) -> dict[str, Any]:
    validate_analysis_lock(analysis_lock, core_csv, entry_csv)
    panels = {
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--analysis-lock", help="Bản khóa đã kiểm duyệt kèm hàm băm dữ liệu và đặc tả")
    return p


def main() -> int:
    args = parser().parse_args()
    result = run(args.core, args.entry, args.output_dir, args.analysis_lock)
